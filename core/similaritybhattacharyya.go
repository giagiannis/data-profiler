...
// newKMeansClustering returns the centroids of k clusters calculated by the
// provided dataset tuples
func newKMeansClustering(k int, tuples []DatasetTuple) [][]float64 {
	algo := kMeansAlgorithm{k, tuples, nil}
	algo.Run()
	// calculated centroids position
	// lorem ipsum etc.
	return nil
}
...
type kMeansAlgorithm struct {
	...
	tuples    []DatasetTuple // the tuples
	centroids [][]float64    // the centroids
}

func (k *kMeansAlgorithm) initializeCentroids() {
	log.Println("Initializing centroids")
	for i := 0; i < k.k; i++ {
		tup := k.tuples[rand.Int()%len(k.tuples)]
		k.centroids[i] = make([]float64, len(tup.Data))
		for j := 0; j < len(tup.Data); j++ {
			k.centroids[i][j] = tup.Data[j]
		}
	}
}

func (k *kMeansAlgorithm) assignTuplesToCentroids() [][]DatasetTuple {
	tupleClusters := make([][]DatasetTuple, k.k)
	for _, t := range k.tuples {
		minDst, minIdx := math.Inf(1), -1
		for j, c := range k.centroids {
			dst, err := norm(t.Data, c, 2)
			if err != nil {
				log.Println(err)
			} else if dst < minDst {
				minIdx = j
		...
		if minIdx > -1 && minIdx < len(tupleClusters) {
			tupleClusters[minIdx] = append(tupleClusters[minIdx], t)
		} else {
			log.Println("Missed a tuple!!")
		}
	}
	return tupleClusters
}

func (k *kMeansAlgorithm) estimateCentroids(clusters [][]DatasetTuple) [][]float64 {
	var centroids [][]float64
	attrs := len(k.tuples[0].Data)
	for _, c := range clusters {
		centroid := make([]float64, attrs)
		for _, t := range c {
			for i := 0; i < attrs; i++ {
				centroid[i] += t.Data[i]
			}
...
func (k *kMeansAlgorithm) centroidsDelta(old, new [][]float64) float64 {
	dst := 0.0
	for i := 0; i < k.k; i++ {
		d, err := norm(old[i], new[i], 2)
		if err != nil {
			log.Println(err)
		} else {
			dst += d
...
func (k *kMeansAlgorithm) Run() {
	k.centroids = make([][]float64, k.k)
	k.initializeCentroids()
	oldDelta := math.Inf(1)
	maxIterations := 100
	for i := 0; i < maxIterations; i++ {
		clusters := k.assignTuplesToCentroids()
		newCentroids := k.estimateCentroids(clusters)
		delta := k.centroidsDelta(newCentroids, k.centroids)
		k.centroids = newCentroids
		log.Printf("k-means iteration %d: oldDelta=%.5f, new delta=%.5f\n", i, oldDelta, delta)
		oldDelta = delta
	}
}
